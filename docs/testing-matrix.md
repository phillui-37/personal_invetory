...
| Android build contract | Debug APK existence/shape plus the local APK-before-tests rule; current CI validates APK packaging in a separate job from `flutter test` | `frontend/android/` + `.github/workflows/mobile-builds.yml` + `docs/build-android.md` | Flutter SDK, Android SDK, Java 17 | `cd frontend && flutter build apk --debug && flutter test test/android_build_test.dart` |
