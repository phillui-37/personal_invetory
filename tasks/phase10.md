...
| Repo task tooling | The repo still points people at raw commands only; there is no checked-in `bin/` entrypoint layer for common verification and task flows yet. | `README.md`, `docs/testing-matrix.md`, missing `bin/` directory |
...
10. The repo has checked-in `bin/` entrypoints for the common verification path, and Windows PowerShell smoke is tracked as host-limited until actually run on the right host.
